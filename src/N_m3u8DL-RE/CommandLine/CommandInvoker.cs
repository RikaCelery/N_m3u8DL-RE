...
internal static partial class CommandInvoker
{
    ...
    private static readonly Option<bool> Status = new("--status") { Description = ResString.cmd_status };
    ...
    private static readonly Option<string?> LivePipeOptions = new("--live-pipe-options") { Description = ResString.cmd_livePipeOptions, HelpName = "OPTIONS" };
    ...
    private static Option<T> WithDefault<T>(this Option<T> option, T defaultValue)
    {
        ...
        // 拼接：原描述 + 空格 + [default: ...]
        option.Description = string.IsNullOrWhiteSpace(currentDesc) ? $"[default: {defaultText}]" : $"{currentDesc.Trim()} [default: {defaultText}]";
        return option;
    }
    ...
    public static async Task<int> InvokeArgs(string[] args, Func<MyOption, Task> action)
    {
        ...
        {
            Input, TmpDir, SaveDir, SaveName, SavePattern, LogFilePath, BaseUrl, ThreadCount, DownloadRetryCount, HttpRequestTimeout, ForceAnsiConsole, NoAnsiColor, Status, AutoSelect, SkipMerge, SkipDownload, CheckSegmentsCount,
        ...
        {
            var msg = Logger.LogLevel == Common.Log.LogLevel.DEBUG ? ex.ToString() : ex.Message;
#if DEBUG
            msg = ex.ToString();
#endif
            Logger.Error(msg);
